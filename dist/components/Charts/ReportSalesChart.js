...
import { getChartByFuelPumpPeriod, getChartByFuelTankPeriod, } from "common/api.js";
...
const ReportSalesChart = () => {
    ...
    const updateChartData = useCallback(async () => {
        ...
        try {
            let data;
            if (type === "sale") {
                data = await getChartByFuelPumpPeriod(controllerId, fuelGrade, pump, period, token);
            }
            else if (type === "purchase") {
                data = await getChartByFuelTankPeriod(controllerId, fuelGrade, tank, period, token);
            }
            const filteredData = {
                labels: [],
                datasets: [
            ...
            if (data) {
                switch (period) {
                    case "weekly":
                        ...
                        break;
                    case "monthly":
                        filteredData.labels = data.map((item) => `${item.dateF}\n${item.nameF}\n(pump${item.pump})`);
                        break;
                    case "yearly":
                        filteredData.labels = data.map((item) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Flex, { marginLeft: "3%" },
            React.createElement(ReportSalesChartMenu, { filter: filter, onChange: handleMenuChange })),
        React.createElement(ReactApexChart, { options: createReportSalesChartOptions(chartData.labels), series: chartData.datasets, type: "bar", width: "100%", height: "150%" }),
        React.createElement(Flex, { justifyContent: "center", color: "white", flexDirection: "row" },
            React.createElement(Text, { marginRight: "10px" },
                "Type: ",
                filter.type),
            React.createElement(Text, { marginRight: "10px" },
                "Fuel Grade: ",
                filter.fuelGrade),
            filter.type === "sale" ? (React.createElement(Text, { marginRight: "10px" },
                "Pump: ",
                filter.pump)) : (React.createElement(Text, { marginRight: "10px" },
                "Tank: ",
                filter.tank)),
            React.createElement(Text, { marginRight: "10px" },
                "Period: ",
