  ...
  useEffect(() => {
    const updateChart = async () => {
      try {
        ...
        const filteredData = {
          categories: fetchedData.map((item: { dateTime: string | number | Date; }) => {
          ...
          series: [
            {
              name: "Tank Level",
              data: fetchedData.map((item: { tankVolumeChanges: any; }) => item.tankVolumeChanges),
            },
          ],
        };
